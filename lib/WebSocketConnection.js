...
class WebSocketConnection extends EventEmitter {
  ...
  close(reasonCode = WebSocketConnection.CLOSE_REASON_NORMAL, description) {
    // Validate input BEFORE creating Promise (synchronous validation)
    if (this.connected) {
    ...
    // Return a Promise that resolves when 'close' event fires (v2.0 feature)
  ...
  sendUTF(data, cb) {
    ...
    // If no callback provided, return a Promise (v2.0 feature)
    if (!cb) {
  ...
  // Async iterator for messages (v2.0 feature)
  async *messages() {
    const messageQueue = [];
