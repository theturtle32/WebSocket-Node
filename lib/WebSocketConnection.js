...
extend(WebSocketConnection.prototype, {
            ...
            case 0x00: // WebSocketFrame.CONTINUATION
                ...
                if (frame.fin) {
                    ...
                    this.frameQueue.forEach(function (currentFrame) {
                        totalLength += currentFrame.binaryPayload.length;
                    });
                    if (totalLength > this.maxMessageSize) {
                        this.drop(WebSocketConnection.CLOSE_REASON_PROTOCOL_ERROR,
                                  "Maximum message size exceeded.");
                        this.frameQueue = [];
                        return;
                    }
                    var binaryPayload = new Buffer(totalLength);
                    this.frameQueue.forEach(function (currentFrame) {
    ...
    sendUTF: function(data) {
        if (this.connected && this.socket.writable && data) {
