...
var setImmediateSupported = ('setImmediate' in global);
...
WebSocketConnection.prototype.processReceivedData = function() {
    ...
    if (this.bufferList.length > 0) {
        if (setImmediateSupported) {
            ...
            setTimeout(this.receivedDataHandler, 0);
        }
    }
};

WebSocketConnection.prototype.handleSocketError = function(error) {
    debug('socket error: %j', error);
...
WebSocketConnection.prototype.handleSocketEnd = function() {
    // FIXME: Process all data still in the receive buffer before fully closing
    // the connection.
    debug('socket end');
    this.frameQueue = [];
    this.outgoingFrameQueue = [];
...
WebSocketConnection.prototype.handleSocketDrain = function() {
    debug('socket drain event');
    this.outputPaused = false;
    this.needsDrainEvent = true;
    this.processOutgoingFrameQueue();
};

WebSocketConnection.prototype.handleSocketPause = function() {
    debug('socket pause event');
    this.inputPaused = true;
    this.emit('pause');
};

WebSocketConnection.prototype.handleSocketResume = function() {
    debug('socket resume event');
    this.inputPaused = false;
    this.emit('resume');
    this.processReceivedData();
};
...
WebSocketConnection.prototype.close = function() {
    debug('Initating clean WebSocket close sequence.');
    // TODO: Should we flush any remaining buffered outgoing messages to the
    // wire before actually starting the close?  Maybe refactor this when we
    // convert to use object mode streams?
    if (this.connected) {
...
WebSocketConnection.prototype.sendCloseFrame = function(reasonCode, reasonText) {
    if (typeof(reasonCode) !== 'number') {
        reasonCode = WebSocketConnection.CLOSE_REASON_NORMAL;
    }

    debug('sendCloseFrame state = %s', this.state);
    ...
    this.writeToSocket(frame.toBuffer(), function(){});
};
...
WebSocketConnection.prototype.processOutgoingFrameQueue = function() {
    if (this.outputPaused || this.state !== STATE_OPEN) { return; }
    if (this.outgoingFrameQueue.length === 0) {
        if (this.needsDrainEvent) {
            this.needsDrainEvent = false;
    ...
    var flushed = this.writeToSocket(buffer, cb);
...
WebSocketConnection.prototype.writeToSocket = function(buffer, cb) {
    try {
        var flushed = this.socket.write(buffer, cb);
        return flushed;
    }
    catch(e) {
        debug('Exception thrown while writing to socket: %s', e);
        if (typeof cb === 'function') {
            process.nextTick(function() {
                cb(e);
            });
        ...
        if (utils.eventEmitterListenerCount(this, 'error') > 0) {
            var self = this;
            process.nextTick(function() {
                self.emit('error', new Error('Error while writing to socket: ' + e.toString()));
            });
            return false;
        }
        return false;
