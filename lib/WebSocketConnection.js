...
WebSocketConnection.prototype.processReceivedData = function() {
    ...
    if (!this.currentFrame.addData(this.bufferList)) {
        if (this.inputPaused) {
            this.inputPaused = false;
            this.socket.resume();
        }
    ...
    // processed.  Also pause the socket so it doesn't flood us with more
    // data until we're done processing what we've got.
    if (this.bufferList.length > 0) {
        ...
        if (setImmediateSupported) {
            ...
            process.nextTick(this.receivedDataHandler);
        }
    }
};
