...
class WebSocketRequest extends EventEmitter {
  constructor(socket, httpRequest, serverConfig) {
    super();
  ...
  readHandshake() {
    const { httpRequest: request } = this;
    ...
    case 13:
      break;
    default: {
      const e = new Error(`Unsupported websocket client version: ${this.webSocketVersion}. Only versions 8 and 13 are supported.`);
      e.httpCode = 426;
      e.headers = {
    ...
    const protocolString = request.headers['sec-websocket-protocol'];
    this.protocolFullCaseMap = {};
    this.requestedProtocols = [];
    if (protocolString) {
      const requestedProtocolsFullCase = protocolString.split(headerValueSplitRegExp);
      requestedProtocolsFullCase.forEach((protocol) => {
        const lcProtocol = protocol.toLocaleLowerCase();
        this.requestedProtocols.push(lcProtocol);
    ...
    if (!this.serverConfig.ignoreXForwardedFor &&
      ...
      const immediatePeerIP = this.remoteAddress;
    ...
    if (this.serverConfig.parseExtensions) {
      const extensionsString = request.headers['sec-websocket-extensions'];
  ...
  accept(acceptedProtocol, allowedOrigin, cookies) {
