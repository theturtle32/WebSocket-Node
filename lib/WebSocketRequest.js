...
function WebSocketRequest(socket, httpRequest) {
    this.socket = socket;
...
extend(WebSocketRequest.prototype, {
    readHandshake: function(request) {
        ...
        if (this.websocketVersion !== '7') {
            throw new Error("Unsupported websocket client version.  Client requested version " + this.websocketVersion + ", but we only support version 7.");
        }
    ...
    accept: function(acceptedProtocol, allowedOrigin) {
        ...
        var connection = new WebSocketConnection(this.socket, [], acceptedProtocol, true);
