...
WebSocketRequest.prototype.accept = function(acceptedProtocol, allowedOrigin, cookies) {
    ...
    if (cookies) {
    	for(var c in cookies){
    		...
    		if(invalidChar) throw new Error("Illegal character " + invalidChar[0] + " in cookie name");
    		...
    		if(cookie.path){
    			invalidChar = cookie.path.match(/[\x00-\x20\x3b]/)
    		...
    		if(cookie.expires && (typeof cookie.expires.getTime !== "function" || typeof cookie.expires.getTime() !== "number")) throw new Error("Invalid date object in cookie expires");
    		
    		//"Max-Age=" non-zero-digit *DIGIT
    		if(cookie.maxage && (isNaN(parseInt(cookie.maxage)) || parseInt(cookie.maxage) <= 0)) throw new Error("Cookie max-age must be a non-zero digit");
    		
    		//"Secure;"
    		if(cookie.secure && typeof cookie.secure !== "boolean") throw new Error("Cookie secure must be of type boolean");
    		
    		//"HttpOnly;"
    		if(cookie.httponly && typeof cookie.httponly !== "boolean") throw new Error("Cookie http only must be of type boolean");
    		
    		response += "Set-Cookie: " + cookie.name + "="  + cookie.value + ";";
    		if(cookie.expires)  response += " Expires=" + new Date(cookie.expires.getTime()).toGMTString() + ";";
    		if(cookie.maxage)  response += " Max-Age=" + parseInt(cookie.maxage) + ";";
