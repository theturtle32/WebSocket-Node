...
extend(WebSocketFrame.prototype, {
    addData: function(bufferList, fragmentationType) {
        ...
        if (this.length > this.maxReceivedFrameSize) {
            ...
            this.dropReason = "Frame size of " + this.length.toString(10) +
                              " bytes exceeds maximum accepted frame size of " +
                              this.maxReceivedFrameSize.toString(10) + " bytes.";
            return true;
        }
        ...
        if (this.parseState === WAITING_FOR_PAYLOAD) {
            if (this.opcode === 0x01 && this.fin) { // Normal Text Frame
            ...
            // is said and done.
            else if ((this.opcode === 0x00 && (fragmentationType === 0x01 || fragmentationType === 0x02)) || // Continuation
                     (this.opcode === 0x02 && this.fin)) // Normal Binary Frame
            {
                if (bufferList.length >= this.length) {
                ...
                switch (this.opcode) {
                    ...
                    case 0x08: //WebSocketOpcode.CONNECTION_CLOSE
                        if (bufferList.length >= this.length) {
                            this.binaryPayload = bufferList.take(this.length);
                            bufferList.advance(this.length);
                            if (this.mask) {
    ...
    toBuffer: function(nullMask) {
        ...
        if (this.opcode === 0x02) { // WebSocketOpcode.BINARY_FRAME
        ...
        else if (this.opcode === 0x08) { // WebSocketOpcode.CONNECTION_CLOSE
            this.length = 2;
            if (this.utf8Payload) {
        ...
        else if (this.opcode === 0x0A && this.binaryPayload) { // WebSocketOpcode.PONG
            data = this.binaryPayload;
            this.length = data.length;
        }
        else if (this.utf8Payload) { // text frames
