...
extend(WebSocketFrame.prototype, {
    addData: function(bufferList, decodeMask, fragmentationType) {
        ...
        if (this.parseState === LOAD_MASK_KEY) {
            if (bufferList.length >= 4) {
                bufferList.joinInto(maskBytes, 0, 0, 4);
                bufferList.advance(4);
                this.maskPos = 0;
                var maskKey = ctio.ruint32(maskBytes, 'big', 0);
                this.parseState = DECODE_HEADER;
            }
        }
        if (this.parseState === DECODE_HEADER) {
            if (bufferList.length >= 2) {
                bufferList.joinInto(frameHeader, 0, 0, 2);
                bufferList.advance(2);
                if (decodeMask) {
                    this.applyMask(frameHeader, 0, 2);                    
                }
                var firstByte = frameHeader[0];
                var secondByte = frameHeader[1];
        ...
        if (this.parseState === WAITING_FOR_16_BIT_LENGTH) {
            if (bufferList.length >= 2) {
                bufferList.joinInto(frameHeader, 2, 0, 2);
                bufferList.advance(2);
                if (decodeMask) {
                    this.applyMask(frameHeader, 2, 2);                    
                }
                this.length = ctio.ruint16(frameHeader, 'big', 2);
                this.parseState = WAITING_FOR_PAYLOAD;
            }
        }
        else if (this.parseState === WAITING_FOR_64_BIT_LENGTH) {
            if (bufferList.length >= 8) {
                bufferList.joinInto(frameHeader, 2, 0, 8);
                bufferList.advance(8);
                if (decodeMask) {
                    this.applyMask(frameHeader, 2, 8);
                }
                var lengthPair = ctio.ruint64(frameHeader, 'big', 2);
                if (lengthPair[0] !== 0) {
        ...
        if (this.parseState === WAITING_FOR_PAYLOAD) {
			...
			if ((this.opcode === 0x00 && fragmentationType === 0x04) || // Continuation with Text Frame
				...
				if (bufferList.length >= this.length) {
					...
					if (decodeMask) {
					    this.applyMask(this.binaryPayload);
					}
					bufferList.advance(this.length);
					this.parseState = COMPLETE;
					return true;
    ...
    applyMask: function(buffer, offset, length) {
        ...
        for (var i=offset; i < end; i++) {
            buffer[i] = buffer[i] ^ maskBytes[this.maskPos];
            this.maskPos = (this.maskPos + 1) & 3;
        }
    },
    toBuffer: function(socket, maskContents, nullMask) {
        var maskKey;
        var headerLength = 2;
        var data;
        var outputPos = 0;
        ...
        if (maskContents) {
            ...
            ctio.wuint32(maskKey, maskBytes, 'big', 0);
        }
