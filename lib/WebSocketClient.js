...
class WebSocketClient extends EventEmitter {
  ...
  connect(requestUrl, protocols = [], origin, headers, extraRequestOptions) {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      self._connectPromiseReject = reject;

      // Set up one-time listeners for this connection attempt
      const handleConnect = (connection) => {
        self._connectPromiseReject = null;
        resolve(connection);
      ...
      self.once('connect', handleConnect);
      self.once('connectFailed', handleConnectFailed);

      // All existing connect logic continues below
      ...
      for (const protocol of this.protocols) {
        for (const character of protocol) {
          ...
          if (charCode < 0x0021 || charCode > 0x007E || protocolSeparators.includes(character)) {
      ...
      req.on('response', (response) => {
        ...
        if (utils.eventEmitterListenerCount(self, 'httpResponse') > 0) {
          ...
          const headerDumpParts = Object.entries(response.headers)
    ...
    // Add empty .catch() to prevent unhandled rejection errors
    // when promise is not awaited (backward compatibility)
    promise.catch(() => {}); // Errors are still emitted via 'connectFailed' event
  ...
  validateHandshake() {
    ...
    if (this.protocols.length > 0) {
      ...
      if (this.protocol) {
        if (!this.protocols.includes(this.protocol)) {
  ...
  failHandshake(errorDescription) {
  ...
  abort() {
    ...
    if (this._connectPromiseReject) {
      // Call reject, but don't let it become an unhandled rejection
      // The connectFailed event still fires, so old code works fine
