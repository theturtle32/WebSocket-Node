...
WebSocketClient.prototype.connect = function(requestUrl, protocols, origin) {
    ...
    if(this.secure) {
        req = this.request = https.request(reqOptions);
        agent = this.agent = https.getAgent({ host: this.url.hostname, port: this.url.port });
    } else {
        req = this.request = http.request(reqOptions);
        agent = this.agent = http.getAgent(this.url.hostname, this.url.port);
    }

    agent.on('upgrade', function(response, socket, head) {
    ...
    agent.on('error', handleRequestError);
